...
updated_answers = [
    st.text_area(f"Answer {i+1}", value=ans, key=f"edit_q{i}", height=70)
    for i, ans in enumerate(answers)
]

# --- Previous Prompt
if "generated_prompt" in profile:
    st.markdown("### 🧬 Last Saved Adaptive Prompt")
    st.code(profile["generated_prompt"])
...
# --- Optional Save Without Regeneration
if st.button("💾 Save Without Regeneration"):
    try:
        save_user_profile(
            name=name,
            age=age,
        ...
        st.success("✅ Profile saved without regenerating prompt.")
    except Exception as e:
